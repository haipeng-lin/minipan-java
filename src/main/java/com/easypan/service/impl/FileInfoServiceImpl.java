...
public class FileInfoServiceImpl implements FileInfoService {
    ...
    /**
     * @Description： 将指定文件id的文件合并成一个文件
     *                1.查找出指定文件id的分片文件的文件夹（根目录+temp+用户id文件id）
     *                2.查找出目标文件夹（根目录+file+年月份）
     *                3.合并分片文件（临时目录路径,目标文件路径）
     */
    @Async
    public void transferFile(String fileId, SessionWebUserDto webUserDto) {
        ...
        try {
            ...
            String tempFolderName = appConfig.getProjectFolder() + Constants.FILE_FOLDER_TEMP;
            String currentUserFolderName = webUserDto.getUserId() + fileId;
            File fileFolder = new File(tempFolderName + currentUserFolderName);         //存放分片文件的临时文件夹
    ...
    /**
     * @Description：
     * @Param： 1：dirPath：临时目录文件
     *          2：toFilePath：目标文件路径
     *          3：delSource：指示在合并文件完成后是否删除原始分片文件的标志，若为true，则删除
     * @Return
     */
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public FileInfo newFolder(String filePid,
                              String userId,
                              String folderName) {
        ...
        fileInfo.setFolderType(FileFolderTypeEnums.FOLDER.getType());   //类型为目录类型
        fileInfo.setCreateTime(curDate);
        fileInfo.setLastUpdateTime(curDate);
